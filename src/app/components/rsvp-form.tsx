...
const RsvpForm: React.FC = () => {
    ...
    const [plusOneValue, setPlusOneValue] = useState<boolean>(false)
    ...
    async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const body = {
            "name": data.get("name"),
            "rsvpStatus": data.get("rsvpStatus") === 'on' ? 'true' : 'false',
            "plusOne": data.get("plusOne") === 'on' ? 'true' : 'false',
        ...
        <div
            ...
            >
                <form action="../api/submit/route.ts" onSubmit={handleSubmit} className="flex flex-col flex-grow max-w-screen-xl gap-6">
                    ...
                    <input type="text" name="name" id="full-name" placeholder="Full Name" value={nameValue} onChange={(e) => setNameValue(e.target.value)} required={true} className="p-2.5 w-full h-12 text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"/>
                    ...
                    <label className="inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="rsvpStatus" id="attending" className="sr-only peer" checked={rsvpStatus} onChange={(e) => setRsvpStatus(e.target.checked)}/>
                        <div
                            className="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300rounded-full peer  peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
                        <span className="ms-3 text-sm font-modernLoveCaps text-gray-900 ">Yes/No</span>
                    </label>
                    <div className="flex flex-col justify-between">
                        <h1 className="font-modernLoveCaps text-3xl">Will you be bringing a plus one?</h1>
                        <label className="inline-flex items-center cursor-pointer">
                            <input name="plusOne" type="checkbox" id="plusOne" className="sr-only peer" checked={plusOneValue} onChange={(e) => setPlusOneValue(e.target.checked)}/>
                            <div
                            ...
                            <span
                                className="ms-3 text-sm text- font-modernLoveCaps text-gray-900">Yes/No</span>
                        </label>
                    </div>
                    ...
                    <input type="email" name="email" id="email" placeholder="your.email@domain.com" value={emailValue} onChange={(e) => setEmailValue(e.target.value)}
                           className="p-2.5 w-full h-12 text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"/>
                    ...
                    <button type="submit"
                            className="text-white text-xl font-modernLoveGrunge bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg px-5 py-2.5 text-center me-2 mb-2">RSVP
                    </button>
                </form>
            </div>
            {footer()}
